...
export const listProposalLaborItems = async (database: Pick<LocalDatabase, 'query'>, proposalId: string) => {
  const result = await database.query<{
    id: string; description: string; professional_count: string; monthly_salary: string;
...
export const createProposalLaborItem = async (database: LocalDatabase, proposalId: string, input: ProposalLaborInput) => {
  await database.transaction(async (transaction) => {
    ...
    `, [randomUUID(), proposalId, next.rows[0]?.position ?? 1, input.description.trim(), input.professionalCount,
...
export const updateProposalLaborItem = async (database: LocalDatabase, proposalId: string, itemId: string, input: ProposalLaborInput) => {
  await database.transaction(async (transaction) => {
    await assertEditable(transaction, proposalId);
    calculateLaborItem(input);
...
export const removeProposalLaborItem = async (database: LocalDatabase, proposalId: string, itemId: string) => {
  await database.transaction(async (transaction) => {
    await assertEditable(transaction, proposalId);
...
export const updateProposalStandardMonthlyHours = async (database: LocalDatabase, proposalId: string, hours: number) => {
  await database.transaction(async (transaction) => {
    await assertEditable(transaction, proposalId);

  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !loading) onClose(); }}>
    <section className="new-proposal-dialog" role="dialog" aria-modal="true" aria-labelledby="new-proposal-title">
      ...
      <form onSubmit={(event) => void submit(event)}>
        <label htmlFor="new-proposal-client"><span><Building2 size={15} /> Cliente <b>*</b></span><select id="new-proposal-client" value={clientId} disabled={loading} onChange={(event) => { const nextId = event.target.value; const nextClient = clients.find((client) => client.id === nextId); setClientId(nextId); setWorkId(nextClient?.works.find((work) => work.active)?.id ?? ''); }}><option value="">Selecione</option>{clients.filter((client) => client.works.some((work) => work.active)).map((client) => <option key={client.id} value={client.id}>{client.tradeName || client.legalName}</option>)}</select></label>
        <label htmlFor="new-proposal-work"><span><MapPin size={15} /> Obra <b>*</b></span><select id="new-proposal-work" value={workId} disabled={loading || !clientId} onChange={(event) => setWorkId(event.target.value)}><option value="">Selecione</option>{activeWorks.map((work) => <option key={work.id} value={work.id}>{work.name}</option>)}</select></label>
        <label htmlFor="new-proposal-scope" className="wide"><span>Escopo da proposta <b>*</b></span><input id="new-proposal-scope" autoFocus type="text" maxLength={300} value={scope} disabled={loading} onChange={(event) => setScope(event.target.value)} /></label>
        <label htmlFor="new-proposal-validity" className="wide"><span><CalendarDays size={15} /> Validade</span><input id="new-proposal-validity" type="date" value={validUntil} disabled={loading} onChange={(event) => setValidUntil(event.target.value)} /></label>

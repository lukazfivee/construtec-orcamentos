...
Validado ao vivo com um mock HTTP local no lugar do Centro de Custos real (nunca tocou producao): aprovar -> sincronizar -> reload completo -> botao mostra "Ir para Centro de Custo" e o deep link (`#obra=4242` no teste) funciona tanto na navegacao interna (iframe) quanto na janela externa. typecheck e test:critical (23/23) ok. Desktop inalterado. Commit `d6e8b87`, deploy `58697d4c` (imagem do container mudou, nao so assets estaticos -- confirmado /health ok apos hibernar).

### Correcao: iframe do Centro de Custos bloqueado por CORS na nuvem

Usuario testou "Ir para Centro de Custo" na nuvem real e caiu na tela "Centro de Custos nao iniciado" (mensagem de `.bat`/Portal Hub, feita pro desktop local). Causa: `CentroCustosWorkspace` gateava o iframe atras de um `fetch('/api/health')` entre origens -- sempre bloqueado por CORS na nuvem (cada app e' um Worker independente), mesmo com o outro servico rodando normalmente. Corrigido usando `isCloudRuntime()` (ja existente) pra pular esse gate fora do desktop Electron; o iframe (que nao sofre CORS, diferente de fetch) carrega direto com o deep link certo. Validado ao vivo (nuvem simulada): iframe renderiza a URL correta em vez da tela de bloqueio. Desktop local inalterado. Commit `0aafb82`.

### Correcao (outro repo): CSP `frame-ancestors` bloqueava o iframe na nuvem

Mesmo apos o fix de CORS acima, o iframe continuava em branco na nuvem real. Causa era outra: `frame-ancestors` e CORS sao mecanismos distintos -- o header `Content-Security-Policy: frame-ancestors` do app EMBEDADO (Centro de Custos) so' permitia `'self'` e origens localhost, entao o navegador recusava silenciosamente renderizar o iframe vindo da origem cloud do Orcamentos. Confirmado via `curl -D -` no header real. Corrigido em `centro de custos CONSTRUTEC/middleware/observability.js` linha 17, adicionando a origem `https://construtec-orcamentos-cloud.construtec-reports.workers.dev` na lista de `frame-ancestors`. Commit isolado (`bdc5b83`, so' esse arquivo) nesse outro repo, autorizado explicitamente pelo usuario apos bloqueio do classificador de permissao ao tentar `git status`/`git branch`. Deploy via `wrangler deploy` no container (Version ID `ce20eeb6`) -- esse deploy tambem levou outras mudancas nao commitadas que ja estavam na working tree desse repo (Docker `COPY` le o filesystem, nao o git; usuario avisado, decisao de formalizar ou nao essas outras mudancas ainda pendente).

### Pivo final: sem iframe, navegacao na mesma janela

Usuario decidiu que nao quer iframe nenhum pro botao principal: "quero simplesmente que abra o app", "com api", "na mesma janela". Removida a rota de iframe desse botao (o componente `CentroCustosWorkspace` continua existindo, so nao e mais usado por este fluxo). `ProposalSyncDirectAction` ganhou `handleGoToCenter()`, que faz `window.location.assign(url)` (navegacao real na mesma aba/janela) na nuvem, ou `window.construtec.openExternal(url)` no desktop Electron (onde nao existe "mesma janela" fora do shell do app). Extraido um helper `centerDeepLinkUrl()` compartilhado com o botao secundario "Janela externa" (que continua abrindo em outra aba de proposito, uso dual monitor). Removido o botao redundante "Visualizar Obra Integrada" do painel de sucesso. Validado ao vivo em instancia isolada com mock local do Centro de Custos: aprovar -> sincronizar -> clicar "Ir para Centro de Custo" -> o mesmo portal navega direto pra `#obra=8181`, sem aba nova, sem iframe. typecheck e test:critical (23/23) ok. Desktop inalterado.